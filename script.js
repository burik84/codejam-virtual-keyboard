...
const KEYBOARD = {
  ...
  createKeys() {
    ...
    const createIconHTML = (iconName, keyName) => `<i class="material-icons">${iconName}</i> ${keyName}`;
      ...
      switch (key) {
        case 'Backspace':
          ...
          keyElement.addEventListener('click', () => {
            this.proper.value = this.proper.value.substring(0, this.proper.value.length - 1);
        ...
        case 'Caps Lock':
          ...
          keyElement.addEventListener('click', () => {
            this.toggleCapsLock();
            keyElement.classList.toggle('keyboard__key--active', this.proper.capsLock);
          });
          break;
        ...
        default:
          keyElement.textContent = key.toLowerCase();
          keyElement.addEventListener('click', () => {
            this.proper.value += this.proper.capsLock ? key.toUpperCase() : key.toLowerCase();
            this.triggerEvent('oninput');
          });
          break;
      }
  ...
  triggerEvent(handlerName) {
    if (typeof this.eventHandlers[handlerName] === 'function') {
      this.eventHandlers[handlerName](this.proper.value);
    }
  },
  toggleCapsLock() {
    this.proper.capsLock = !this.proper.capsLock;
    this.elements.keys.forEach((el) => {
      const key = el;
      if (key.childElementCount === 0) {
        if (this.proper.capsLock) {
  ...
  open(initialValue, oninput) {
    this.proper.value = initialValue || '';
...
document.onkeypress = function keyHandlers(event) {
  ...
  KEYBOARD.proper.value += elementKey;
  outTextBox.value = KEYBOARD.proper.value;
};
